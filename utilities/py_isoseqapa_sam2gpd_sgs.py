...
def convert(inputs):
	...
	if (rname != "*") and ("M" in cigar) and ("H" not in cigar): # aligned and non-hard-clipped reads
		...
		gpd_output = "\t".join([qname,qname,rname,strand,str(int(pos)-1),str(exon_end),mapq,sf,str(len(exon_start_list)-1),",".join(exon_start_list),",".join(exon_end_list),flag,seq])
	else:
		gpd_output = None
	return gpd_output
